...
#Making Selectivity tables for easier visualization

#Vanderploeg and Scavia Index of Electivity (E*) Table
E_star_wide <- Eelectivity_and_Selectivity %>%
  dplyr::select(Year, Region, Site, Period, Prey_Type, E_star) %>%
...
#Strauss's Linear Index Table
Li_wide <- Eelectivity_and_Selectivity %>%
  dplyr::select(Year, Region, Site, Period, Prey_Type, Li) %>%
  pivot_wider(names_from = Prey_Type, values_from = Li)%>%
  dplyr::select(Year, Region, Site, Period, Fish, Shrimp, Crayfish, Odonata, Hemiptera, Coleoptera, Gastropoda, Diptera)%>%
  mutate(across(Fish:Diptera, ~replace_na(as.character(round(.x, 4)), "+NA")))
View(Li_wide)

#T-tests on Strauss Linear index values to test significance of selection (or avoidance)
...
Linear_Index_Graph <- ggplot(LI_summary_2, aes(x = prey_type, y = value)) + 
  ...
  geom_point(aes(y = mean_LI), 
             shape = 1, fill = NA, color = "black", size = 1.5, stroke = 0.5) +
...
ggsave("Linear_Index_Graph2.eps", Linear_Index_Graph, width = 3.307, height = 3, dpi = 800)
```
...
##Crayfish
```{r}
#FITTING THE MODEL (all sizes of eels)
...
add1(Crayfish_glm,
     scope = ~ TL_mm + TT_Mean_Count_Crayfish + TT_Mean_Count_Fish + TT_Mean_Count_Hemiptera + TT_Mean_Count_Odonata +TT_Mean_Count_Shrimp,test = "Chisq")
...
#MODEL WITH LARGE EELS ONLY (>250mm TL)
#Full model
...
##Fish
```{r}
#Full model with all eels of all sizes
...
##Odonata
```{r}
#Fitting the model (with all eels of all sizes)
...
# Stepwise selection (backward)
drop1(Hemiptera_glm, test = "Chisq")
...
#MODEL WITH LARGE EELS ONLY (>250mm TL)
#Full model
Hemiptera_glm = glm(Eel_Presence_Hemiptera ~ TL_mm+ TT_Mean_Count_Crayfish + TT_Mean_Count_Fish +TT_Mean_Count_Hemiptera + TT_Mean_Count_Odonata+ TT_Mean_Count_Shrimp, family = "binomial", data = eels2_large)
summary(Hemiptera_glm)
...
#Final (reduced ) model
Hemiptera_glm = glm(Eel_Presence_Hemiptera ~ TT_Mean_Count_Crayfish + TT_Mean_Count_Odonata, family = "binomial", data = eels2_large)
summary(Hemiptera_glm)
vif(Hemiptera_glm)
